...
#define CUS_EVT_FW_RECOVER_START "EVENT=FW_RECOVER_START"
		if (!strncmp((char *)event, CUS_EVT_FW_RECOVER_START,
			     strlen(CUS_EVT_FW_RECOVER_START))) {
			printf("EVENT: FW_RECOVER_START\n");
			break;
		}
#define CUS_EVT_FW_RECOVER_SUCCESS "EVENT=FW_RECOVER_SUCCESS"
		if (!strncmp((char *)event, CUS_EVT_FW_RECOVER_SUCCESS,
			     strlen(CUS_EVT_FW_RECOVER_SUCCESS))) {
			printf("EVENT: FW_RECOVER_SUCCESS\n");
			break;
		}
#define CUS_EVT_FW_RECOVER_FAIL "EVENT=FW_RECOVER_FAILURE"
		if (!strncmp((char *)event, CUS_EVT_FW_RECOVER_FAIL,
			     strlen(CUS_EVT_FW_RECOVER_FAIL))) {
			printf("EVENT: FW_RECOVER_FAILURE\n");
			break;
		}
#define CUS_EVT_CHAN_SWITCH_COMPLETE "EVENT=CHANNEL_SWITCH_COMPLETE"
		if (!strncmp((char *)event, CUS_EVT_CHAN_SWITCH_COMPLETE,
			     strlen(CUS_EVT_CHAN_SWITCH_COMPLETE))) {
			printf("%s\n", (char *)event);
			break;
		}
		...
		if (!strncmp((char *)event, CUS_EVT_MLAN_CSI,
			...
			if ((fstat == 0) &&
				...
				if (chmod(filename, S_IRUSR | S_IWUSR |
							    ...
							    S_IROTH |
							    S_IWOTH)) {
					printf("ERR: Unable to change file permissions\n");
				}
			}
			break;
		}
