...
/** Max retry for OBSS scan after association*/
t_u8 max_obss_retry;
...
static void run_app(int nl_sk)
{
	...
	while (!terminate_flag) {
		...
		if (assoc_flag && is_ht_ap) {
			...
			if (evt_conn) {
				while (max_obss_retry < MAX_OBSS_SCAN_RETRY) {
					/** Issue OBSS scan */
					max_obss_retry++;
					ret = process_setuserscan();
					if (ret == MLAN_STATUS_SUCCESS) {
						/** Invoke 2040coex command, if
						 * any legacy AP found or
						 *  any AP has 40MHz intolarent
						 * bit set */
						if (coex_cmd_req_flag)
							invoke_coex_command();
						break;
