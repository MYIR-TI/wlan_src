...
#define CCA_THREHOLD_MAX 120
#define CCA_THREHOLD_MIN 1
...
static char *usage[] = {
	...
	"         delts", "         dfstesting", "         clear_nop",
	"         nop_list", "         fake_radar", "         dfs53cfg",
	"         dfs_mode", "         dfs_cac", "         autodfs",
	...
	"         cwmode", "         txrxhistogram", "         per_pkt_cfg",
	"         dot11_txrx", "         robustcoex", "         ctrldeauth",
	"         dmcs", "         range_ext", "         twt_setup",
	"         twt_teardown", "         rx_abort_cfg",
...
static int process_ch_load(int argc, char *argv[])
{
	...
	if (cl_cfg->duration > MAX_CH_LOAD_DURATION || cl_cfg->duration < 1)
		cl_cfg->duration = MAX_CH_LOAD_DURATION;
	if (argc == 5) {
		cl_cfg->cca_th = atoi(argv[4]);
		if (cl_cfg->cca_th > CCA_THREHOLD_MAX ||
		    cl_cfg->cca_th < CCA_THREHOLD_MIN) {
			printf("ERR: threshold value not in range (1 to 120)\n");
			goto done;
		}
	}
	printf("action=%d duration: %d cca_th: %d\n", cl_cfg->action,
	       cl_cfg->duration, cl_cfg->cca_th);
...
static int process_cwmode(int argc, char *argv[])
{
	...
	printf("CW Mode:\nMode = %d\nChannel = %d\nChanInfo = 0x%x\n"
	       "TxPower=%d\nRateInfo=0x%x\nPacket Length=%d\n",
	       cwmode->mode, cwmode->channel, cwmode->chanInfo, cwmode->txPower,
	       cwmode->rateInfo, cwmode->pktLength);
