		       ...
		       ieeetypes_subband_set_t *sub_bands, t_u8 *pdomain_code)
{
	...
	if (fp == NULL) {
		printf("File (config/80211d_domain.conf) opening Error\n");
		return UAP_FAILURE;
	}
	...
	if (pdomain_code && no_of_sub_band) {
		*pdomain_code = parse_domain_name(domain_name);
		printf("Read domain name=%s domain_code=%d band=%d no_of_sub_band=%d\n",
		       domain_name, *pdomain_code, band, no_of_sub_band);
	}
...
{
	printf("\nUsage : skip_cac <channel> <mode> <bw>\n");
...
static int apcmd_skip_cac(int argc, char *argv[])
{
	...
	if ((IS_HEX_OR_DIGIT(argv[2]) == UAP_FAILURE) ||
	    ((atoi(argv[2]) < 0) || (atoi(argv[2]) > 3) ||
	     (atoi(argv[2]) == 1))) {
		printf("ERR: Invalid bandwidth.\n");
		print_skip_cac_usage();
		return UAP_FAILURE;
	}
	param.action = 1;
	...
	param.bw = (t_u8)A2HEXDECIMAL(argv[2]);
 ...
 *  $return         N/A
 */
static void print_antcfg_usage(void)
{
 ...
 *  @return         UAP_SUCCESS/UAP_FAILURE
 */
static int apcmd_antcfg(int argc, char *argv[])
{
	int opt;
	int tx_val = 0;
	int rx_val = 0;
 ...
 *  $return         N/A
 */
static void print_htstreamcfg_usage(void)
{
	printf("\nUsage : htstreamcfg [<n>]\n");
	printf("\n        Where  <n>  ");
 ...
 *  @return         UAP_SUCCESS/UAP_FAILURE
 */
static int apcmd_htstreamcfg(int argc, char *argv[])
{
	int opt;
...
static int apcmd_sys_config_profile(int argc, char *argv[])
{
	...
	while (config_get_line(line, MAX_CONFIG_LINE, config_file, &li, &pos)) {
		...
		if (strcmp(args[0], "AuthMode") == 0) {
			...
			memset((t_u8 *)tlv, 0, sizeof(tlvbuf_auth_mode));
			cmd_len += tlv_len;
			/* Set TLV fields */
			tlv->tag = MRVL_AUTH_TLV_ID;
			tlv->length = sizeof(tlvbuf_auth_mode) - TLVHEADER_LEN;
...
{
	reg_domain_param_t regDomainInfo;
	...
	if (argc) {
		...
		if ((output[1][0] != -1) && (output[1][1] > output[1][0])) {
			...
			no_of_sub_band = parse_domain_file(
				country, band, sub_bands, &domain_code);
			if (no_of_sub_band == UAP_FAILURE) {
				printf("Parsing Failed\n");
				return UAP_FAILURE;
			}
	...
	if (cflag) {
		...
		if (domain_code) {
			...
			regDomainInfo.length = uap_cpu_to_le16(
				sizeof(reg_domain_param_t) - BUF_HEADER_SIZE);
			regDomainInfo.regDomain = uap_cpu_to_le16(domain_code);

			memcpy(((t_u8 *)&cmd_buf->domain +
				(cmd_buf->domain.length + TLVHEADER_LEN)),
			       &regDomainInfo,
			       regDomainInfo.length + TLVHEADER_LEN);
			cmd_len += regDomainInfo.length + TLVHEADER_LEN;
		}
	}
...
#define JP_REGION_0xFE 0xFE
	...
	if ((!acs_mode_enabled && (channel_tlv_band == BAND_A)) ||
		...
		if (bss_config->bandcfg.chanBand == BAND_5GHZ) {
			...
			if (update_domain_info(country_80211d, BAND_A) ==
			    UAP_FAILURE) {
				return UAP_FAILURE;
			}
		}
